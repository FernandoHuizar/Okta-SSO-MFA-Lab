...
3. **MFA lockout** — Simulated a user losing access to their enrolled authenticator. Resolved using Okta's Reset Authenticators admin action, allowing the user to re-enroll.<img width="585" height="536" alt="image" src="https://github.com/user-attachments/assets/2750c473-ba16-4f06-b19e-e31844f92f63" />
...
5. **OIDC redirect URI mismatch** — Intentionally misconfigured the OIDC app's registered redirect URI, reproducing a real `invalid_request` / `redirect_uri mismatch` error (HTTP 400) during the OAuth authorization flow. Resolved by correcting the registered URI to match the client application's expected callback.<img width="1221" height="880" alt="image" src="https://github.com/user-attachments/assets/4881ff94-9362-4a7e-a97a-641c44600347" />
